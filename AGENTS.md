...
TDD is opt-out: `driver.tdd_mode` (config, default true; set via `--update-config`) is the single source. The `--tdd`/`--no-tdd` flags were removed in 5.0.0. The value is frozen once into `state.tdd_mode` at init; `state.red_phase` tracks whether the one-time RED gate is still pending — it is resolved by a genuine RED confirmation (`red_test` fails). Choosing TDD vs non-TDD is a **plan-time** decision (7.0.0): the planner classifies the work and recommends `driver.tdd_mode` — TDD for genuinely-new behavior whose test fails first, non-TDD for regression/maintenance/existing-behavior work (there is no in-flow "these tests target existing behavior, skip RED" escape any more).
...
- `test_implementation` → `red_test` while `red_phase` (unconditionally — the test author's mandatory-since-6.6.0 result is read only for the die-on-missing guard; routing does not depend on its content). A red-phase authoring pass is always a genuinely-new feature whose tests must prove RED first: as of 7.0.0 there is no in-flow "skip RED" path (`red_expected` was removed — existing-behavior work is a plan-time non-TDD decision). Else (not `red_phase`, a test-repair pass) it reads the same mandatory result and routes by `blocked_on`: `status:"blocked"` + `blocked_on:"implementation"` → `implementation` (6.8.0 — the test author verified its tests correct and says the production code is the gap; symmetric to the implementor's `blocked_on:"tests"` reroute below, and like it takes **no** counter bump, staying bounded by the `implementation → test_implementation` edge), every other result (`implemented`, or `blocked` with `contract`/`tests`) → `test`
- `red_test` reuses `dp-tester` but inverts the meaning: a **failing** run = RED confirmed → `implementation` (and `red_phase` flips to false). A **passing** run = RED not confirmed (the tests are vacuous — existing-behavior work is a non-TDD run as of 7.0.0, so a passing red_test in a TDD run means weak/non-asserting tests) → re-author, incrementing `iterations.test_implementation`; `failure_type: environment` halts.
...
- Counters (`iterations.test`, `iterations.review`, `iterations.test_implementation`) are independent and never reset within a run. The off-by-one is intentional and unchanged: `max_X_iteration = N` permits N+1 attempts (`> max` check). `iterations.test_implementation` is shared by two distinct triggers — a red-phase vacuous-test re-author (`red_not_confirmed`) and an implementor's `blocked_on:"tests"` reroute from `implementation` — both are "retry authoring the tests" in spirit, but a run that has already spent most of the budget on one has less left for the other.
...
- **`driver run-stage`** — always assembles `(system = dp-<role>.md, user = stage-input.json inputs)` and persists them, then executes/validates per the runner's mode:
  - **Bash runner.** Substitutes placeholders (`{system_file}` `{user_file}` `{output_file}` `{project_root}` …, shell-quoted) and runs it with `cwd=work_root`. (The `{project_root}` *placeholder name* is unchanged for existing `RUNNERS.md` templates, but its substituted value is `work_root` — the isolated worktree checkout under `--worktree`.) Capped by the runner's `timeout` only if one is set (unbounded otherwise); combined stdout+stderr streams live to `<work_dir>/<role>-runner.log` (6.2.0) so the SKILL/user can observe a long-running CLI mid-execution.
  - **Validation by category (6.6.0: `category` and `schema` are independent axes in `ROLE_META`).** `category` drives boundary/manifest handling; `schema` drives JSON-result validation.
    - **json roles** (tester/reviewer, `category:"json"`): result written to `{output_file}` → `normalizer` (`default` = tolerant, the default / `passthrough` = strict) → schema.
    - **file roles** (implementor/test_implementor, `category:"file"`): now ALSO carry a `schema` (`implementor-result`) for their status JSON, validated the identical way once `returncode == 0`. A normalizer on a file role is still rejected (its status JSON always uses `default`), and its git delta remains what boundary-checks/manifest-records.
  - **File-role status JSON is REQUIRED, not a second result channel.** `implementor`/`test_implementor` get an `{output_file}` at `<iter_dir>/<role>-result.json` alongside the git delta (their primary content result). A bash runner that exits 0 but fails to produce a valid one is NOT `ok:true` (one error-fed retry for this failure mode only — a plain nonzero exit fails immediately, no retry). It lets a role report `status:"blocked"` + a `concern` when the contract can't be satisfied, instead of grinding with no way to say so (see `states/implementation.md`/`states/test_implementation.md` — the empty-delta guard checks this FIRST so a deliberate "blocked, no changes" isn't mistaken for "didn't run"). The implementor's `status:"blocked"` may also carry `blocked_on:"tests"` (TDD only) — `cmd_advance`'s `implementation` branch routes that to `test_implementation` instead of `test` (see "Key transition rules").
  - **main-session/subagent runner.** The driver (a subprocess) can't call the host's Task tool or main session, so it **hands off**: emits `{mode:"main-session"|"subagent", system_file, user_file, output_file, model?, compact_first?}`, the SKILL executes it (see `SKILL.md §Role Execution`), then validates via `driver finalize-stage` (the same normalize→schema→persist-canonical path bash JSON roles get). (The 5.0.0 removal of `spec_author` removed the `"named"` category; `init` validates the plan body deterministically, not a runner.)
  - **Output directive is command-shape-aware, not CLI-aware.** A command whose template already references `{output_file}` (a stdout redirect like claude's `>`, or a CLI-native result flag like codex's `-o`) is told to give the JSON as its final answer only (captured automatically); a command with no `{output_file}` reference (e.g. cline) is told to write it to that exact path itself — keeping bash-runner prompts as close to identical as the CLIs allow.
  - **File-role fallback is not working-tree-isolated** — keep file roles single-runner unless partial edits from a failed early runner are acceptable. **Unaffected by `--worktree`:** worktree isolation is *from `project_root`'s own working tree*, not *between a role's own fallback runners* — every runner for a role (including fallbacks) shares the same `work_root`, so a partial edit from a failed early runner is exactly as visible to its fallback as it always was.
...
└── iterations/<n>/
    ...
    ├── test-result.json
    ├── review-result.json
    ├── implementor-result.json        # required (6.6.0+): implementor's status (implemented|blocked) + concern, alongside its code delta
    ├── test_implementor-result.json   # required (6.6.0+): same, for the test author (TDD only) — shares implementor-result.schema.json
