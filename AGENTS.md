...
└── iterations/<n>/
    ...
    ├── test-result.json
    ├── review-result.json
    ├── implementor-result.json        # optional: implementor's status (implemented|blocked) + concern, alongside its code delta
    ├── test_implementor-result.json   # optional: same, for the test author (TDD only) — shares implementor-result.schema.json
