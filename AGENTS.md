...
- `test_implementation` → `red_test` while `red_phase` **and** the test author's (mandatory since 6.6.0) result isn't `status:"implemented"` + `red_expected:false`; → `test` directly if it is (6.7.0 — skips RED confirmation entirely, since these tests target pre-existing behavior, and flips `red_phase` to false; a `status:"blocked"` result is unaffected by `red_expected` and still falls through to `red_test` unconditionally, same as before 6.7.0); else (not `red_phase`) → `test` (a test-repair pass)
