...
TDD is opt-out: `driver.tdd_mode` (config, default true; set via `--update-config`) is the single source. The `--tdd`/`--no-tdd` flags were removed in 5.0.0. The value is frozen once into `state.tdd_mode` at init; `state.red_phase` tracks whether the one-time RED gate is still pending — it is resolved either by a genuine RED confirmation (`red_test` fails) or by the test author declaring `red_expected: false` (6.7.0; see below) on an authoring pass whose tests all target pre-existing behavior.
...
- `test_implementation` → `red_test` while `red_phase` **and** the test author's (mandatory since 6.6.0) result isn't `status:"implemented"` + `red_expected:false`; → `test` directly if it is (6.7.0 — skips RED confirmation entirely, since these tests target pre-existing behavior, and flips `red_phase` to false; a `status:"blocked"` result is unaffected by `red_expected` and still falls through to `red_test` unconditionally, same as before 6.7.0); else (not `red_phase`, a test-repair pass) it reads the same mandatory result and routes by `blocked_on`: `status:"blocked"` + `blocked_on:"implementation"` → `implementation` (6.8.0 — the test author verified its tests correct and says the production code is the gap; symmetric to the implementor's `blocked_on:"tests"` reroute below, and like it takes **no** counter bump, staying bounded by the `implementation → test_implementation` edge), every other result (`implemented`, or `blocked` with `contract`/omitted/`tests`) → `test`
- `red_test` reuses `dp-tester` but inverts the meaning: a **failing** run = RED confirmed → `implementation` (and `red_phase` flips to false). A **passing** run = RED not confirmed (vacuous tests) → re-author, incrementing `iterations.test_implementation`; `failure_type: environment` halts. On that re-author pass the test author may instead resolve the ambiguity itself via `red_expected: false` (6.7.0, see above) rather than re-triggering this loop — re-entry is in fact the primary case that field exists for, since a passing re-authored test is exactly as likely to mean "targets existing behavior" as "still vacuous."
