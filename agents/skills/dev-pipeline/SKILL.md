...
5. **After `driver run-stage`, read its JSON.** A `mode` of `main-session`/`subagent` means "execute this yourself" ([§Role Execution](#-role-execution)); otherwise `ok: true` → proceed, `ok: false` with `reason: "all_runners_failed"` → stop and report the `attempts`. For a **json role** (tester/reviewer) bash runner, run-stage already wrote and schema-validated the result file — you do **not** run `validate-result`. (After a main-session/subagent **json** role you DO run `driver finalize-stage` to normalize + validate — that is the handoff's equivalent, and the only time you touch validation.) **Exception:** for a **file role** (implementor/test_implementor) bash runner, `judge()` does not schema-validate its optional result-status JSON — you still run `validate-result --type implementor|test_implementor` yourself, exactly as `states/implementation.md`/`states/test_implementation.md` Step 3 direct, regardless of runner mode.
...
- `run_dir`, `contract_path` — returned by `driver init` (`contract_path` = the plan body the roles read; **`plan_path` is NOT fed to the roles**). On `--resume`, `driver resume` returns these (plus `project_dir`, `plan_path`, `tdd_mode`, `work_root`, `worktree_branch`, `worktree_base_ref`) from `state.json` — it replays the current state's landing echo, persisted to `<run_dir>/last-advance.json` on every transition.
...
- **file role** (`category: "file"` — implementor / test_implementor): the executor edits files in `work_root` (the driver appends an absolute-root directive to the assembled `user_file` — "Operate from this absolute project root … resolve every relative path against it: `<work_root>`" — telling the executor exactly where that is). When it finishes, **first check whether the echoed `output_file` (a result-status JSON — `<iter_dir>/implementor-result.json` or `<iter_dir>/test_implementor-result.json`) exists and is schema-valid** (`driver validate-result --type implementor|test_implementor --file <path>`; a non-zero exit here is advisory only — see the relevant state file, do not apply Global Rule 6 to this specific call). If it validates with `status: "blocked"`, treat this as the role's DELIBERATE outcome — an empty or partial delta is expected and correct, **do not** apply the empty-delta guard below; relay the `concern` to the user instead (see `states/implementation.md` / `states/test_implementation.md`). Otherwise (no result file, or `status: "implemented"`), compute the delta as the state file does. **An empty delta with no `blocked` status means the role did not run** — re-execute once, stating that nothing was produced; if still empty, stop and report (the handoff equivalent of `all_runners_failed`). Then continue the state's boundary/manifest steps.
