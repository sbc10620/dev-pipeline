...
5. **After `driver run-stage`, read its JSON.** A `mode` of `main-session`/`subagent` means "execute this yourself" ([§Role Execution](#-role-execution)); otherwise `ok: true` → proceed, `ok: false` with `reason: "all_runners_failed"` → stop and report the `attempts`. For a **bash** runner, run-stage already wrote and schema-validated the result file — you do **not** run `validate-result`. (After a main-session/subagent role you DO run `driver finalize-stage` to normalize + validate — that is the handoff's equivalent, and the only time you touch validation. Since 6.6.0 this applies uniformly to a **json role**'s result [tester/reviewer] and a **file role**'s now-mandatory status JSON [implementor/test_implementor] alike — the git delta is still the file role's primary content result, but its status JSON is validated identically to a json role's.)
...
11. **Finishing a role's own output is not the end of the state — for any role, in any state.** After a main-session/subagent role produces its result (including an *approving* review), you must still complete that state file's remaining steps through `driver advance` and open the next `states/<next_state>.md` before your turn ends (see [§Role Execution](#-role-execution)'s "you are the orchestrator again"). An approving review does not end the run — only `driver advance` does, and it still routes to `states/done.md`, which has real work left (commit, worktree merge, self-evolution). This rule is about not mistaking a role's finished output for a finished state — it is **not** license to blow past a genuine human-in-the-loop stop, whether that is an **approval gate** (e.g. `states/planning.md`'s default-flow plan sign-off — skipped under `--auto-run`) or a **runtime safety confirmation** (e.g. `states/review.md`'s main-session reviewer question — *not* an approval gate, and *not* skipped by `--auto-run`). Where a state file explicitly tells you to STOP and wait for the user, that stop is real: hold there until it resolves.
...
- `plan_path` — the plan.md path: written by the planner (`--request`, saved under `.dev-pipeline/plans/<YYYYMMDD>-<slug>.md` by default — see `states/planning.md`), or given by the user (`--plan` / `--update-config`) at whatever path they chose.
...
- `run_dir`, `contract_path` — returned by `driver init` (`contract_path` = the plan body the roles read; **`plan_path` is NOT fed to the roles**). On `--resume`, `driver resume` returns these (plus `project_dir`, `plan_path`, `tdd_mode`, `work_root`, `worktree_branch`, `worktree_base_ref`, and — only when `--summary`/`--summary-file` was passed — `task_summary`) from `state.json` — it replays the current state's landing echo, persisted to `<run_dir>/last-advance.json` on every transition.
...
- `work_root` — returned by `driver init` **and re-echoed by every `driver advance`**, just like `tdd_mode`: the directory where code is edited and where every state file's git bookkeeping (baseline, delta, review diff, commit) must run. Under `--worktree` this is the worktree checkout, not `project_root`; otherwise the two are identical. **Always use the latest echoed `work_root` for `git -C ...` / `cd ...` in a state file — never `project_root`, and never re-derive it.** `project_root` keeps its own meaning below (the `.dev-pipeline`/config/`.agents` home) and is never the git bookkeeping root under `--worktree`.
...
**Observing a bash runner while it runs.** A bash runner streams its combined stdout+stderr in real time to `<iter_dir>/<role>-runner.log` (`run-stage` truncates it fresh at the start of the call; the path is deterministic from `iter_dir`, which you already have from the advance echo — you don't need to wait for `run-stage`'s completion JSON to know it). **If your host can run a shell command in the background and periodically re-check a file, do that for every bash-runner `run-stage` call:** launch it in the background, then check the log's tail every ~30–60s. Note the truncation happens a moment into `run-stage`'s own startup (after it resolves the runner config), not at the instant you launch it — if you check within the first fraction of a second of backgrounding the call, you may briefly see a prior call's leftover log content; a check a moment later will reflect the truncated, current-run log. **Some runners write to that log constantly and some barely write to it at all — both are normal, not a sign of success or failure** (which CLI/role combination does which is documented per-CLI in `RUNNERS.md`, not something you need to know in advance): e.g. a read-only reviewer whose command redirects its own stdout to the result file writes almost nothing to the log until it exits. Treat a quiet log as expected, not stuck — a runner's `timeout` is optional and **unbounded when unset** (no default cap), so unless the configured runner sets `timeout` explicitly, the driver will NOT kill a hung runner for you; a quiet log is not proof of progress either way, so don't infer a hang from silence alone.

**Keep checking cheap, but throttle what you relay to the user** — a log check is a single file read and costs little, but composing a "still running" note each cycle adds up over a multi-minute call, especially for a runner whose log rarely changes. So: check every ~30–60s as above, but only **relay** a note when the tail shows genuinely new content since your last relay; if it hasn't changed, relay a brief "still running" at most every 2–3 checks (~90–180s) rather than every single one — enough to keep a multi-minute call from leaving the session silent, without composing a message every cycle. A runner whose log never grows will naturally settle into that sparser relay cadence on its own; one that streams continuously will naturally relay more often (only when there's something new to say) — you don't need to know which CLI you're running for this to work out right. When `run-stage` finishes, read its JSON as usual — it also echoes `log_file` for reference. **If your host cannot run in the background, run `run-stage` in the foreground as before** — the log is still written in real time, so a user watching a separate terminal can `tail -f <iter_dir>/<role>-runner.log` even though you can't check it yourself.
...
- **file role** (`category: "file"` — implementor / test_implementor): the executor edits files in `work_root` (the driver appends an absolute-root directive to the assembled `user_file` — "Operate from this absolute project root … resolve every relative path against it: `<work_root>`" — telling the executor exactly where that is) and writes its status JSON to the echoed `output_file` (`<iter_dir>/implementor-result.json` or `<iter_dir>/test_implementor-result.json`) — this file is **mandatory since 6.6.0**, validated the same way a json role's result is. When it finishes, do these three in order, then continue the state's boundary/manifest steps:
