...
11. **Finishing a role's own output is not the end of the state — for any role, in any state.** After a main-session/subagent role produces its result (including an *approving* review), you must still complete that state file's remaining steps through `driver advance` and open the next `states/<next_state>.md` before your turn ends (see [§Role Execution](#-role-execution)'s "you are the orchestrator again"). An approving review does not end the run — only `driver advance` does, and it still routes to `states/done.md`, which has real work left (commit, worktree merge, self-evolution). This rule is about not mistaking a role's finished output for a finished state — it is **not** license to blow past a genuine human-approval pause. Where a state file explicitly tells you to STOP and wait for the user (e.g. `states/planning.md`'s default-flow plan sign-off, or `states/review.md`'s main-session reviewer question), that stop is a real gate: hold there until the user replies (each such gate documents its own `--auto-run` behavior).
