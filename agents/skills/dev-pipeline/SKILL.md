...
- **file role** (`category: "file"` — implementor / test_implementor): the executor edits files in `work_root` (the driver appends an absolute-root directive to the assembled `user_file` — "Operate from this absolute project root … resolve every relative path against it: `<work_root>`" — telling the executor exactly where that is) and writes its status JSON to the echoed `output_file` (`<iter_dir>/implementor-result.json` or `<iter_dir>/test_implementor-result.json`) — this file is **mandatory since 6.6.0**, validated the same way a json role's result is. When it finishes, run `driver finalize-stage --run <run_dir> --role <role> --stage-input <stage_input_path>` (the same command a json role's handoff gets) to normalize + schema-check it: `ok: true` → proceed; `ok: false` → re-execute **once**, appending a `## Your previous output was REJECTED` section with the `problem`; if it fails again, stop and report. Then read the (now guaranteed-valid) file: `status: "blocked"` is the role's DELIBERATE outcome — an empty or partial delta is expected and correct, **do not** apply the empty-delta guard below; relay the `concern` to the user instead (see `states/implementation.md` / `states/test_implementation.md`). Otherwise (`status: "implemented"`), compute the delta as the state file does. **An empty delta with `status: "implemented"` means the role did not run** — re-execute once, stating that nothing was produced; if still empty, stop and report (the handoff equivalent of `all_runners_failed`). Then continue the state's boundary/manifest steps.
