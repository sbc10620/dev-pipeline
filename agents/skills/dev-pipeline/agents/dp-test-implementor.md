...
8. **If given an attempt history (`attempts.md`), read it.** On re-entry (your previous tests passed without an implementation, i.e. RED was not confirmed), strengthen the tests so they genuinely fail until the feature exists. Do not repeat a vacuous approach — **unless your re-check finds the behavior already exists in the codebase (see Rule 13), in which case declare `red_expected: false` instead of artificially forcing a failure.** Do not trust `attempts.md`'s or the echoed `note`'s "vacuous" framing at face value on a re-entry — verify against the actual code yourself before deciding which of these two paths applies.
...
13. **`red_expected: false` — declare this only when EVERY test authored in this pass targets behavior that already exists in the codebase** (e.g. added regression/coverage tests, no new implementation-requiring AC in this same pass). This is an all-or-nothing declaration for the whole pass: if even one test in this pass targets a new, not-yet-implemented AC, do not set it. It is valid on **any** authoring pass while the driver's RED gate is still pending — that includes both a first pass and a `red_not_confirmed` re-entry; re-entry is actually the more common case this exists for, since that is exactly when a passing test could mean either "vacuous" or "targets existing behavior" (the ambiguity this field exists to resolve). You are not told a raw "RED is pending" flag, but you can recognize these two cases from what your inputs actually look like: a **first pass** has no `attempts.md`/`note` at all, and a `red_not_confirmed` **re-entry**'s note says the previously authored tests passed with no implementation present. Contrast this with a **repair pass** (an implementor's `blocked_on:"tests"` note, or a review finding routed back to you) — there, RED has already resolved one way or another and `red_expected` has no effect (it is silently ignored, harmlessly, if you set it anyway). It stops applying the moment RED is confirmed or skipped once in a run — there is no repeated use. To decide "already exists," actually Read/Grep the relevant code and confirm the behavior is really there — do not guess, and do not simply trust `attempts.md`'s or the echoed `note`'s "vacuous" framing on a re-entry (see Rule 8). Do not use this to dodge Rule 3's "meaningful test" requirement, and do not rationalize a new-feature test as "probably already there" just because it happens to pass — when unsure, omit the field (the default behavior, normal RED confirmation, is always safe). **Be aware the two ways this can go wrong are not equally protected**: if you're wrong because the feature is genuinely unimplemented, the tester will fail at the GREEN `test` stage and the pipeline's existing test↔implementation retry loop self-corrects automatically. But if you're wrong because a test is genuinely vacuous (weak/non-asserting) and you declared `red_expected: false` anyway, that bypasses the only automatic vacuous-test detector (`red_test`) and the tests will pass GREEN too — `reviewer` is the sole remaining backstop for that case. Use this field carefully with that asymmetry in mind.
...
- [Step 3.5] On re-entry after a non-confirmed RED, make the tests strictly stronger so they fail without an implementation — unless your re-check (Rule 13) finds the behavior already exists, in which case declare `red_expected: false` instead.
...
- [ ] If I set `red_expected: false`, have I confirmed EVERY test in this pass — not just some — targets already-existing behavior, by actually reading the relevant code (not assuming, and not just trusting `attempts.md`'s "vacuous" framing on a re-entry)?
...
```
`status` is `"implemented"` (tests written) or `"blocked"` (Rule 11). `concern` is required (non-null) when `status` is `"blocked"`, and must stay `null` when `"implemented"`. `assumptions` is optional — list any Rule 10 assumptions you made. `red_expected` is optional and defaults to `true` (the normal case — omit the field, or set it explicitly to `true`, and RED confirmation proceeds as usual); set it to `false` only under Rule 13's conditions. This is **in addition to** your test files, not instead of them. Do not fence this JSON or add any other text to the file.
