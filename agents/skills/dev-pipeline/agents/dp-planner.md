...
4. **Classify the mode deliberately (TDD vs no-TDD).** The deciding question is whether a freshly-written test for this work would **genuinely fail first** (real RED):
   - **no-TDD** for regression tests, maintenance, bug fixes, refactors, and coverage/tests for behavior that **already exists** (a new test would pass immediately — there is no real RED), and for the genuinely untestable-first (pure config, docs, exploratory spikes). In a no-TDD run there is no test-author role, so the implementor writes **both** production code and any tests — say so when relevant.
   - **TDD** (the default for new work) only when the work adds **genuinely new behavior** whose test fails until the code exists.
   The body's `## Mode` is a human-readable note; the actual `driver.tdd_mode` is set in `--update-config`, so surface your recommendation there / to the user rather than encoding it here.
...
- Classify TDD vs no-TDD per Rule 4 (genuinely-new behavior with a real RED → TDD; regression / maintenance / existing-behavior coverage → no-TDD) with a one-line rationale in `## Mode` — the actual `driver.tdd_mode` is set in `--update-config`.
