...
Report:
- Which budget was exhausted. The `outcome` in `state.json` history distinguishes them: `test_fail_exhausted` (green test), `review_fail_exhausted` (review), `red_not_confirmed_exhausted` (the authored tests kept passing with no implementation — the tests are likely vacuous; point the user at the test author output), or `implementor_blocked_on_tests_exhausted` (an author↔implementor standoff over whether the tests or the production code is wrong never converged — either side may have driven it: the implementor via `status:"blocked", blocked_on:"tests"`, or the test author via `status:"blocked", blocked_on:"implementation"`; point the user at both sides' concerns in `attempts.md`). The latter two share the `test_implementation` iteration budget, so exhaustion may reflect either failure mode.
- The last failure details / review findings, and a summary of all attempts from `attempts.md`.
- If the advance echoed a `hint` (TDD review exhaustion, or an author↔implementor standoff), surface it **as-is** — it already names what to inspect (e.g. that the blocking findings may point at tests that contradict the contract rather than a production defect, or that both the tests and the production code need inspecting).
