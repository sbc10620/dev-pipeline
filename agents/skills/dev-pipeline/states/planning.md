...
- [Step 2] **Validate the plan before showing it** (the planner is not driver-validated, so this is the parity gate). Run:
  ```bash
  python3 <driver_path> validate-config --config <project_root>/.dev-pipeline/dev-pipeline.config.json --plan <plan_path>
  ```
  This merges the plan's header (full whitelist) and checks the merged config **and** the plan body's required sections exactly as `init` will.
  - `valid: true` → continue to [Step 3].
  - Non-zero exit → **bounded repair loop:** read the reported errors, revise `plan.md` (follow `dp-planner.md`) to fix exactly those, and re-run this step. After **3** attempts without success, **stop** and ask the user for the missing information — do not push a broken plan forward.
