...
The advance that landed here echoed `directive: run_reviewer`, `iter_dir`, `contract_path`, `changes_diff` (the path the reviewer will read), `reviewer_config`, and **`work_root`** — plus, only if a `red_expected: false` skip happened earlier in this run (TDD), `red_confirmation_skipped_note` (6.7.0): a flag telling the reviewer some tests never went through RED confirmation and need extra scrutiny (see `dp-reviewer.md`). The driver persisted the reviewer context to `<iter_dir>/stage-input.json`, with `output_file` set to `<iter_dir>/review-result.json`.
