...
**Observability (6.2.0, refined 6.2.1).** A bash runner's combined stdout+stderr streams live to `<iter_dir>/<role>-runner.log` — `run-stage` echoes the path as `log_file`, and on a host that supports backgrounding, the SKILL runs the CLI in the background and checks that log periodically so a multi-minute LLM call doesn't leave the session silent (see `SKILL.md §Role Execution`). What actually shows up there is CLI-dependent — some runners write to it constantly, some barely at all (both normal, not a sign of success or failure); `RUNNERS.md` documents which is which per CLI/role. The SKILL relays a status note to the user only when there's new log content to report (or, on a quiet log, at a sparser fallback cadence) rather than on every check, to avoid spending tokens narrating "still running" every cycle.
